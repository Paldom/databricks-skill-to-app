...
- Consumers run `npx skills add Paldom/databricks-skill-to-app` — the CLI
  ...
  users can also `gh skill install Paldom/databricks-skill-to-app <skill>
...
gh repo edit Paldom/databricks-skill-to-app --visibility public --accept-visibility-change-consequences
...
gh api repos/Paldom/databricks-skill-to-app/private-vulnerability-reporting --method PUT

# 3. Cut a versioned release (matches .claude-plugin/plugin.json) + topics
gh skill publish --tag v0.1.0
gh repo edit Paldom/databricks-skill-to-app --add-topic skills-sh
#    (gh < 2.90 has no `gh skill`; equivalent fallback:
#     gh release create v0.1.0 --generate-notes
#     gh repo edit Paldom/databricks-skill-to-app --add-topic agent-skills --add-topic skills-sh)
...
npx skills@latest add Paldom/databricks-skill-to-app --list
mkdir -p /tmp/skills-verify && cd /tmp/skills-verify
env -u DISABLE_TELEMETRY -u DO_NOT_TRACK npx skills@latest add Paldom/databricks-skill-to-app --skill '*' -a claude-code -y
...
curl -sIL https://skills.sh/Paldom/databricks-skill-to-app | head -1
```
...
(`https://skills.sh/b/Paldom/databricks-skill-to-app`) linking to the repo's
skills.sh page.
